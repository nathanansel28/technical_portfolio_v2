...
represent the value of each dependent variable and

```math
\beta_1, \beta_2, ..., \beta_n
```

represent the coefficient of each dependent variable (which represents how significant each feature's contribution towards crop yield is).
...
which enables us to predict crop yield while producing the least amount of error. This process is called model training.
...
By finding the optimal solution, the gradient descent algorithm essentially finds the best possible values of

```math
\beta_1, \beta_2, ... \beta_n
```

which predicts crop yield effectively.
